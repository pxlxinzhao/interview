...
@Controller
public class CheckDatingInboxController {

	@RequestMapping("/html/dating")
    public String register( @RequestParam(value="type", required=true) String type, 
		...
		for(Identity_message j : Im){
			...
			entry.setHeadIcon("people photos/"+sender+".png");
//			entrylist.add(entry); 
			stack.push(entry);
		}
		...
		if(type.equalsIgnoreCase("dating")){
			System.out.println("Entering Dating Inbox");
			return "DatingMailInbox";
		}
